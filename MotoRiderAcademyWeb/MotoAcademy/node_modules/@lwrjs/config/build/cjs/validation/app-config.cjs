...
function validateBootstrap(node, validationContext, propPrefix) {
  if (node) {
    ...
    validationContext.assertArrayOfBootstrapServices((0, import_jsonc_parser.findNodeAtLocation)(node, ["services"]), `${propPrefix}.services`);
...
function validateStaticSiteGeneratorConfig(node, validationContext) {
  ...
  if (node) {
    validationContext.assertValidKeys(node, "staticSiteGenerator", import_app_config_context.STATIC_SITE_GENERATOR_ATTRIBUTE_KEYS);
  }
}
...
function validateRoot(node, validationContext, preMerge) {
  ...
  validateStaticSiteGeneratorConfig((0, import_jsonc_parser.findNodeAtLocation)(node, ["staticSiteGenerator"]), validationContext);
...
function validateLwrAppConfig(config, phase) {
  ...
  try {
    ...
    if (errors.length) {
      ...
      throw (0, import_diagnostics2.createSingleDiagnosticError)({
