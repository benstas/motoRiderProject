...
var LwrAssetRegistry = class {
  ...
  async getAsset(assetId, runtimeEnvironment, includeMetadata) {
    ...
    if (rawAsset.noTransform) {
      ...
      import_diagnostics.logger.debug(`[LwrAssetRegistry][getAsset] no-transform source ${assetId.specifier} -> ${assetDef.uri}`);
      return assetDef;
    }
    const overrideFilePath = this.getHashedAssetPath(rawAsset, runtimeEnvironment);
    if ((0, import_fs.existsSync)(overrideFilePath)) {
      if (includeMetadata && (0, import_fs.existsSync)(`${overrideFilePath}.meta.json`)) {
        rawAsset.metadata = JSON.parse((0, import_fs.readFileSync)(`${overrideFilePath}.meta.json`).toString());
      }
    ...
    if (transformResult) {
      (0, import_fs.writeFileSync)(overrideFilePath, transformResult.source, "utf-8");
      if (transformResult.metadata) {
        (0, import_fs.writeFileSync)(`${overrideFilePath}.meta.json`, JSON.stringify(transformResult.metadata), "utf-8");
      }
      import_diagnostics.logger.debug(`[LwrAssetRegistry][getAsset] cached transformed asset ${assetId.specifier}`);
    } else {
      rawAsset.noTransform = true;
    }
    const newDef = await this.getAssetDefinition(rawAsset, runtimeEnvironment);
    import_diagnostics.logger.debug(`[LwrAssetRegistry][getAsset] fully processed ${assetId.specifier} -> ${newDef.uri}`);
    if (includeMetadata) {
  ...
  getHashedAssetPath({ownHash, entry, type}, runtimeEnvironment) {
    ...
    import_diagnostics.logger.debug(`[LwrAssetRegistry][getHashedAssetPath] ${runtimeHash} -> ${hashedPath}`);
    return hashedPath;
  }
  ...
  async resolveAssetUri(assetId, runtimeEnvironment) {
    ...
    if ((0, import_shared_utils.isExternalUrl)(specifier)) {
      import_diagnostics.logger.debug(`[LwrAssetRegistry][resolveAssetUri] external url ${specifier}`);
      return {
        artifactType: "asset",
        uri: specifier,
        entry: specifier,
    ...
    import_diagnostics.logger.debug(`[LwrAssetRegistry][resolveAssetUri] getUriFromAsset ${specifier} -> ${ret.uri}`);
    return ret;
  }
  async resolveAssetUriWithoutLinking(assetId, runtimeEnvironment) {
    const {specifier} = assetId;
    if ((0, import_shared_utils.isExternalUrl)(specifier)) {
      import_diagnostics.logger.debug(`[LwrAssetRegistry][resolveAssetUriWithoutLinking] external url ${specifier}`);
      return {
        artifactType: "asset",
        uri: specifier,
        entry: specifier,
    ...
    import_diagnostics.logger.debug(`[LwrAssetRegistry][resolveAssetUriWithoutLinking] getUriFromAsset ${specifier} -> ${assetDef.uri}`);
    return uri;
  }
